...
contract NomadBar is Ownable {
  ...
  //@dev You can get the owner address with a Cocktail's id.
  mapping (uint => address) cocktailIdToOwner;
  //@dev You can get the amount of stars with the owner address.
  mapping (address => uint) starsCount;
  //@dev You can get the amount of cocktails with the owner address.
  mapping (address => uint) cocktailsCount;

  //@dev Structure a Cocktail.
  struct Cocktail {
    string name;
    uint look;
  }
  ...
  //@dev Modifier to check cocktails how many the owner's got.
  modifier toastCheck {
  ...
  function createRandomCocktail(string _name) external {
  ...
  function _createRandomDesign(string _str) private view returns(uint) {
    uint randLooks = uint(keccak256(now, msg.sender, _str));
    return randLooks % randModule;
  }
 
 //@dev Creates a cocktail.
  function _createCocktail(string _name, uint _look) private {
  ...
  function _bartend(uint _rand) private returns(uint) {
      randNonce++;
      uint randNum = uint(keccak256(now, msg.sender, randNonce)) % _rand + 1;
      return randNum;
  }
  ...
  function getAllCocktails() external view onlyOwner() returns(uint[]) {
